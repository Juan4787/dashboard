-- Tighten commercial access defaults after the initial compatibility migration.
-- Existing businesses were explicitly backfilled as permanent in
-- 20260528052136_business_subscriptions_access.sql. From this point forward,
-- a missing/new subscription must not silently become permanent/free access.
...
as $$
	select case
		when exists (
			...
			select b.created_at < timestamp with time zone '2026-05-28 05:21:36+00'
			from businesses b
