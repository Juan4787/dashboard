...
-- Los turnos activos cuyo horario ya pasó se conservan en la búsqueda solo
-- durante tres meses calendario. El estado persistido no se modifica: la
-- aplicación los presenta como "Expirado" según starts_at.
...
begin
	...
	from (
		...
		where appointment.business_id = p_business_id
			...
			and appointment.starts_at >= v_now - interval '3 months'
			and (
